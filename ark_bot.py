...
data_list = []
print("🌋 V6.0 ARK 빙의 모드 가동! (혁신 테마에 -200점 폭격 시작)\n")

for i, ticker_str in enumerate(tickers, 1):
    print(f"[{i:03d}/{len(tickers)}] 🔍 {ticker_str: <5} 스캔 중...")
    ...
    try:
        ...
        if type(industry) == str and ('Aerospace' in industry or 'Semiconductor' in industry or 'Software' in industry):
            theme_bonus = -200  # 🚨 일반 기업은 절대 따라올 수 없는 압도적 어드밴티지!
            print(f"    ㄴ 🚀🚀🚀 {ticker_str}: 잭팟! 하이퍼 혁신주 발견! (-200점 버프 부여)")
            
        # --- 1. 가치 & 2. 우량 (비중 대폭 축소 예정) ---
        ...
        gross_margin = info.get('grossMargins', -999) or -999 
        debt = info.get('debtToEquity', 9999) or 9999
            
        # --- 3. 모멘텀 & 성장 (가장 중요한 핵심 지표!) ---
...
print("\n⚙️ 스캔 완료! 아크 인베스트 스타일의 극단적 가중치로 점수를 냅니다...")
...
# --- ⚖️ 극단적 비중 조절 (가치/우량은 반토막, 성장은 2배!) ---
weight_value = 0.5    # "현재 싼 건 중요하지 않아!"
weight_quality = 0.5  # "현재 적자여도 상관없어!"
weight_momentum = 2.0 # "매출이 떡상하고 있는지가 전부야!"

df['Total_Score'] = (df['Rank_Value'] * weight_value) + \
                    (df['Rank_Quality'] * weight_quality) + \
                    (df['Rank_Momentum'] * weight_momentum) + \
                    df['Theme_Bonus'] # 🚨 여기에 -200점 강제 부스터!
...
excel_filename = "US_ARK_Mode_V6.xlsx"
final_df.to_excel(excel_filename, index=False)

print(f"\n✅ V6.0 완성! 세상을 바꿀 광기의 1위 종목은 [{final_df.iloc[0]['Ticker']}] 입니다!")
